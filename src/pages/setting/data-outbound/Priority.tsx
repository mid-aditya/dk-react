...
const Priority: React.FC = () => {
  ...
  const handleEdit = (priority: PriorityItem) => {
  ...
  const handleDelete = (priority: PriorityItem) => {
    setSelectedPriority(priority);
  ...
  const confirmDelete = () => {
    if (selectedPriority) {
      setPriorities((prev) => prev.filter((priority) => priority.id !== selectedPriority.id));
  ...
  const handleSubmit = () => {
    if (!formData.name.trim()) {
      alert("Nama Priority harus diisi");
      return;
    }
  ...
  const tableColumns: TableColumn<PriorityItem>[] = [
    {
      key: "no",
      header: "No",
      align: "left",
    ...
    <WorkspaceLayout>
      <div className="w-full h-full flex flex-col bg-[var(--bg-primary)] overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-secondary)] m-0">
                Manage outbound priority configurations
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="bg-[var(--bg-secondary)] rounded-2xl p-6">
              ...
              {filteredPriorities.length > 0 ? (
                ...
                <div className="flex flex-col items-center gap-2 py-12">
                  <Star className="w-12 h-12 text-[var(--text-tertiary)]" />
                  <p className="text-[var(--text-secondary)] font-medium">
                    Tidak ada priority yang tersedia
                  </p>
                  <p className="text-sm text-[var(--text-tertiary)]">
                    {searchQuery
                      ? "Coba ubah kata kunci pencarian"
                      : "Buat priority baru untuk memulai"}
                  </p>
                </div>
              )}
            </div>
        ...
        <Modal
          ...
          title={selectedPriority ? "Edit Priority" : "Tambah Priority Baru"}
          size="md"
          footer={
            <div className="flex justify-end gap-3">
              ...
              >
                {selectedPriority ? "Simpan Perubahan" : "Simpan"}
              </button>
            </div>
          }
        >
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-[var(--text-primary)] mb-2">
                Nama Priority <span className="text-[var(--error-color)]">*</span>
              </label>
              <input
                type="text"
                value={formData.name}
                ...
                placeholder="Masukkan nama priority"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--text-primary)] mb-2">
