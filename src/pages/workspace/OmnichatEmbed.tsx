...
const OmnichatEmbed: React.FC = () => {
  ...
  // Detection for customer mode and calling state from URL
  const { isCustomer, initialCalling } = React.useMemo(() => {
    const params = new URLSearchParams(window.location.search);
    return {
      isCustomer: params.has('phone'),
      initialCalling: params.has('calling')
    };
  }, []);
  ...
  if (isCalling && initialCalling) {
    return (
      <CallUI 
        ...
        onSaveTicket={() => {
          console.log('Ticket Saved:', { ticketStatus, ticketNote });
          window.close();
        }}
        isCustomer={false}
      />
  ...
  return (
    <div className={`${theme} w-full h-screen p-0 m-0 flex gap-0 overflow-hidden bg-[var(--bg-primary)] min-w-0 box-border relative`}>
      {/* Card 1 - Inbox */}
      {!isCustomer && (
          ...
          <div className="flex-1 overflow-y-auto p-2">
            ...
            >
              <div className="w-10 h-10 flex-shrink-0 relative overflow-visible">
                <div className="w-10 h-10 rounded-full relative overflow-hidden shadow-[0_2px_8_rgba(0,0,0,0.15)] bg-[var(--bg-tertiary)]">
                  <img 
      ...
      <div className="flex flex-col overflow-hidden min-w-0 bg-[var(--bg-secondary)] border-r border-[var(--border-color)] h-full relative flex-1 min-w-[400px] last:border-r-0">
        <div className="p-3 px-4 border-b border-[var(--border-color)] bg-[var(--bg-secondary)] flex-shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-[var(--text-primary)] m-0">{isCustomer ? "Support Agent" : maskName("Wafiy Ulhaq")}</h3>
                <p className="text-xs text-[var(--text-secondary)] m-0">{isCustomer ? "Online" : maskHandle("@wafiy.ulhaq")}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {isCustomer && !isCalling && (
                <button 
                  onClick={() => {
                    const url = window.location.href + (window.location.search ? '&' : '?') + 'calling=true';
                    window.open(url, 'Support Call', 'width=450,height=700,menubar=0,toolbar=0,location=0,status=0');
                  }}
                  className="w-9 h-9 rounded-full bg-green-600 text-white border-none cursor-pointer flex items-center justify-center hover:bg-green-700 transition-all shadow-sm"
                  title="Call Support in New Window"
                >
              ...
              {!isCustomer && !isCalling && (
                <button 
                  onClick={() => {
                    const url = window.location.href + (window.location.search ? '&' : '?') + 'calling=true';
                    window.open(url, 'Agent Call Center', 'width=1000,height=750,menubar=0,toolbar=0,location=0,status=0');
                  }}
                  className="w-9 h-9 rounded-full bg-green-600 text-white border-none cursor-pointer flex items-center justify-center hover:bg-green-700 transition-all shadow-sm"
        ...
        {/* Agent Call Center View Removed - Now Full Page Popup */}

        {/* Floating Call Card (Customer View Removed - Now Full Page) */}

        {!isCustomer && <TimelineTicket tickets={timelineTickets} />}
